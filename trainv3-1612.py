...
# the hyper-parameters are taken by gridsearch CV. The codes are in the trainv3-params.py file 
models10 = {
...
'PUPIL': Pipeline(steps=[('scaling', StandardScaler()),
                ('reduce_dim', SelectKBest(k=20, score_func=mutual_info_classif)),
                ('classify', SVC(C=5, degree=5, gamma=0.1, probability=True))]),
'GAZE': Pipeline(steps=[('scaling', StandardScaler()),
                ('reduce_dim', SelectKBest(k=6,score_func=mutual_info_classif)),
                ('classify', SVC(C=5, degree=2, gamma=0.1, probability=True))])
}
...
models21 = {
    'EEG': Pipeline(steps=[('scaling', StandardScaler()),
                ...
                ('classify', SVC(C=5, degree=2, gamma=0.1, probability=True))]),
'MOTION': Pipeline(steps=[('scaling', StandardScaler()),
                           ('reduce_dim', SelectKBest(k=13,score_func=mutual_info_classif)),
                ('classify', SVC(C=5, degree=2, gamma=0.1, probability=True))]),
'EDA': Pipeline(steps=[('scaling', StandardScaler()),
                ('reduce_dim',NeighborhoodComponentsAnalysis(n_components=3)),# SelectKBest(k=31,score_func=mutual_info_classif)),
                ('classify', SVC(C=5, degree=2, gamma=0.1, probability=True))]),
      'PUPIL': Pipeline(steps=[('scaling', StandardScaler()),
                ('reduce_dim', NeighborhoodComponentsAnalysis(n_components=3)),
                ('classify', SVC(C=1, degree=2, gamma=0.001, probability=True))]),
...
if __name__ == '__main__':
    scoring = {"ROC": 'roc_auc_ovr', "Accuracy": make_scorer(accuracy_score), "F1-macro": make_scorer(f1_score, average='macro'),
    ...
    for (tw, models) in [('1-0', models10), ('4-2', models42), ('2-1', models21)]:     
        ...
        for key, file in datafiles.items():   
            ...
            outer_cv = cross_validate(pipe, X, y, \
                                      n_jobs=10, cv=logo, groups=groups, scoring=scoring, \
            ...
            outer_cv.update({'y_pred': y_pred, 'y_true': y_true})
            if SAVE_FILE == 1:
